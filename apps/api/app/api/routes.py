...
@router.get('/tickets')
def tickets(page:int=1,page_size:int=50,status:str|None=None,priority:str|None=None,paginated:bool=False,user=Depends(require('ticket:read'))):
...
def knowledge(q: str|None=None,page:int=1,page_size:int=50,paginated:bool=False,user=Depends(require('knowledge:read'))): return _page(search_articles(q) if q is not None else list_articles(),page,page_size,'id','desc',paginated)
...
def insights(user=Depends(require('analytics:read'))): return insights_dashboard()
